...
import com.github.ulwx.aka.gateway.filters.ResponseCode;
 ...
 * @see ErrorAttributes
 */
public class MyDefaultErrorAttributes implements ErrorAttributes {
	private static Logger log = Logger.getLogger(MyDefaultErrorAttributes.class);
	private static final String ERROR_INTERNAL_ATTRIBUTE = MyDefaultErrorAttributes.class.getName() + ".ERROR";
	...
	private Map<String, Object> getErrorAttributes(ServerRequest request, boolean includeStackTrace) {
		...
		errorAttributes.put("timestamp", CTime.formatWholeAllDate());
		...
		errorAttributes.put("status", 0);//errorStatus.value()
		errorAttributes.put("error", ResponseCode.GATEWAY_ERROR.getCode() );
		Object obj=determineMessage(error, responseStatusAnnotation);
		errorAttributes.put("message",ResponseCode.GATEWAY_ERROR+ "["+errorStatus.getReasonPhrase()+"]"+ obj==null?"":obj);
